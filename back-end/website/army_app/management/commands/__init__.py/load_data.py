...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--version", type=str, default="current", help="Version folder (timestamp or name)")
    ...
    def handle(self, *args, **options):
        ...
        # Grab CSV folder based on version
        if options["version"] == "currrent":
            ...
            VERS_DIR = DATA_DIR / options["version"]

        # Rollback
        if options["rollback"]:
            rollback_version = options.get("version") or get_previous_version(DATA_DIR)
            ...
            logger.info(f"Rolling back to previous version: {rollback_version}")
        ...
        all_objects = self.validate_from_version(loaders)
        if options["validate_only"]:
            return
        
        if options["--apply"]:
            logger.info(f"[Apply] Starting data load for version {VERS_DIR}")
        ...
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        logger.info("=" * 80)
        logger.info(f"Ending data validation run at {timestamp}")
        logger.info("=" * 80)

    def rollback_version(self, version_dir):
        logger = logging.getLogger("load_data")
        logger.info(f"Rolling back to version: {version_dir}")
        ...
        logger.info(f"✅ Rollback to {version_dir} completed.")

    def validate_from_version(loaders):
        """
        Validate data from specified version directory
        """
        all_errors = []
